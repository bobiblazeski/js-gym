const DDPG = (function () {
  ...
  if (NODEJS) {
    ({API, N, Util, ReplayBuffer, tf, OUNoise} = {
      API: require('../lib/api'),
      N: require('nial'),
      Util: require('../lib/util'),
      ReplayBuffer: require('../lib/replay.buffer'),
      //tf: require('@tensorflow/tfjs-node'),  
      tf: require('@tensorflow/tfjs-node-gpu'),
      OUNoise: require('../lib/ounoise'),
    });    
  }
  ...
  class DDPG {
    ...
    step (stepNo, prevState, action, reward, observation, done) {
  ...
  const play = async (env, maxEpisodes, render, saved) => {        
    ...
    const agent = new DDPG(inputSize, outputSize);
    await agent.load(saved);
    for (const epNo of N.til(maxEpisodes)) {
      let epReward = 0, observation, reward, done;      
      let stepsTaken  = 0;
      observation = await API.environmentReset(instanceId);
      for (const stepNo of N.til(maxSteps)) {
        let action = await agent.act(observation, false).data();
        action = Array.from(action);
        ({observation, reward, done} =
          await Util.stepResponse(instanceId, action, render));    
        console.log(reward);
        epReward += Math.max(reward, -1);
        ++stepsTaken;
        if (done) break;
      }
      console.log(`epNo: ${epNo} Reward: ${epReward} stepsTaken: ${stepsTaken}`);
    }
